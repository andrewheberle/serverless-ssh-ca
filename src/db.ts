...
export const revocationStatus = async (serial: bigint, publickey: string, certificateType: CertificateType): Promise<RevocationStatus> => {
    const stmt = env.DB
        .prepare("SELECT public_key, revoked_at as count FROM certificates WHERE serial = ? AND public_key = ? AND certificate_type = ?")
        .bind(`${serial}`, publickey)
    ...
    if (res.results[0].revoked_at !== null) {
		return RevocationStatus.Revoked
	}

	if (publickey === res.results[0].public_key) {
...
export const revokeCertificate = async (serial: bigint): Promise<RevokedCertificate[]> => {
    const stmt = env.DB
