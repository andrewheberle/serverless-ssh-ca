...
var _ client.LoginHandler = (*DeviceLoginHandler)(nil)
...
const (
	DefaultLifetime = time.Hour * 24 * 90
)

// NewLoginHandler creates a new handler
func NewLoginHandler(config config.Config, opts ...DeviceLoginHandlerOption) (*DeviceLoginHandler, error) {
...
func (lh *DeviceLoginHandler) HasPrivateKey() bool {
	return lh.config.HasPrivateKey()
}

// GenerateKey will generate a new SSH private key
func (lh *DeviceLoginHandler) GenerateKey() error {
...
func (lh *DeviceLoginHandler) Start(address string) error {
	lh.mu.Lock()
	defer lh.mu.Unlock()
...
// HTTP service is stopped via [*DeviceLoginHandler.Shutdown()].
//
// If the service has not been started this will return [ErrNotStarted]
func (lh *DeviceLoginHandler) Wait(ctx context.Context) error {
	if !lh.mu.TryRLock() {
		return client.ErrAlreadyStarted
	}
	defer lh.mu.RUnlock()
...
func (lh *DeviceLoginHandler) Shutdown() error {
...
// RunPageantProxy will just block in this case
func (lh *DeviceLoginHandler) RunPageantProxy(ctx context.Context) error {
...
func (lh *DeviceLoginHandler) Login(w http.ResponseWriter, r *http.Request) {
...
// SSH CA
func (lh *DeviceLoginHandler) Callback(w http.ResponseWriter, r *http.Request) {
	defer func() {
...
func (lh *DeviceLoginHandler) doLogin(token *oauth2.Token) error {
	// extract id token
...
func (lh *DeviceLoginHandler) Refresh() error {
	// try refresh token
...
func (lh *DeviceLoginHandler) doSigningRequest(access, id string) (*client.CertificateSignerResponse, error) {
...
func (lh *DeviceLoginHandler) ExecuteLogin(addr string) error {
...
// [context.Background()]
func (lh *DeviceLoginHandler) ExecuteLoginWithContext(ctx context.Context, addr string) error {
	return lh.executeLogin(ctx, addr)
}

func (lh *DeviceLoginHandler) executeLogin(ctx context.Context, addr string) error {
...
func (lh *DeviceLoginHandler) HasCertificate() bool {
	_, err := lh.config.GetCertificateBytes()
	return err == nil
}

func (lh *DeviceLoginHandler) CertificateValid() bool {
	//
...
func (lh *DeviceLoginHandler) CerificateExpiry() time.Time {
	certBytes, err := lh.config.GetCertificateBytes()
	if err != nil {
		return time.Time{}
	}
...
// SetLogger sets the [*slog.Logger] used after the [*DeviceLoginHandler] has been
// created by [NewHandler]
func (lh *DeviceLoginHandler) SetLogger(logger *slog.Logger) {
	lh.logger = logger
}

// OIDCConfig shows the current underlying OIDC config
func (lh *DeviceLoginHandler) OIDCConfig() config.ClientOIDCConfig {
	return lh.config.Oidc()
}

// CertificateAuthorityURL shows the CA URL
func (lh *DeviceLoginHandler) CertificateAuthorityURL() string {
